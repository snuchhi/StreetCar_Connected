# this file is changed to meet the needs as per the reuqirement for the StreectCar connected project
#the changes are as follows
# the function _get_corner_points accepts 2 additional parameters groupId and groupedtask distances
# the groupId is the number of the groups thathave been passed to the system.
# The groupedtasks is distance between each tasks 
# the groupedtasks distance is passed as a parameter when the _gantt is called through the main code
# example call for the grouped task dist is below 
# groupedtask = [0,-160,-240]
#fig_pdx_gantt_multiple = ff.create_gantt(df_final, colors=colors, index_col='Resource', show_colorbar=False,
#                     group_tasks=True,groupedtasks_dist = groupedtask,title='TimeSpace Diagram',
...
def _get_corner_points(x0, y0, x1, y1,groupID,groupedtasks_dist):
    """
    ...
    """
    groupdict = dict(enumerate(groupedtasks_dist))
    if groupID in groupdict:
        value = groupdict[groupID]
        return ([x0, x1, x1, x0], [y0+value, y0+value, y1+value, y1+value])
    else:
        return ([x0, x1, x1, x0], [y0, y0, y1, y1]) 
  # for item in list:
   #     return ([x0, x1, x1, x0], [y0+item, y0+item, y1+item, y1+item])
  ...
  #      if(groupID == k):
   #        print("Inside for loop")
    #       print(groupID)
     #      print(k)
      #     print(v)
       #    return ([x0, x1, x1, x0], [y0+v, y0+v, y1+v, y1+v])   
       # else:
        #   return ([x0, x1, x1, x0], [y0, y0, y1, y1]) 
    ...
    #   return ([x0, x1, x1, x0], [y0, y0, y1, y1])
    #elif(groupID == 1):
    #   return ([x0, x1, x1, x0], [y0+100, y0+100, y1+100, y1+100])
    #else:
    #    return ([x0, x1, x1, x0], [y0+200, y0+200, y1+200, y1+200])
